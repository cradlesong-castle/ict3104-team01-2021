...
public class CarControllerBackup : MonoBehaviour
{
    ...
    void Update()
    {
        Debug.Log((prevRotationY) + " " + (waypointNavigator.currentWaypoint.transform.eulerAngles.y) + " " + (Vector3.Distance(prevTransform, transform.position)) /
            (Vector3.Distance(prevTransform, waypointNavigator.currentWaypoint.transform.position)));
        ReactToEnvironment();
        correctRotation = Mathf.Lerp(prevRotationY, waypointNavigator.currentWaypoint.transform.rotation.y, (Vector3.Distance(prevTransform, transform.position)) /
            (Vector3.Distance(prevTransform, waypointNavigator.currentWaypoint.transform.position)));
